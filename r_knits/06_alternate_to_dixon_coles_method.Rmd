...
doDCPrediction<-function(df, xi=0, currentDate=Sys.Date()){
    ...
    res <- optim(par=par.inits, fn=DCoptimFn, DCm=dcm, xi=xi, currentDate=currentDate, method='Nelder-Mead')
...
CMlogLik <- function(y1, y2, lambda, mu, rho=0, sigma=1, weights=NULL){
    ...
    #y1 home goals
    #y2 away goals
    sig<-10^sigma #Optimizing to log sigma to keep it positive. I'll have to compare this to limiting sigma with method="L-Nelder-Mead-B" lower=c(-Inf, -Inf, 0, rep(-Inf, length(params)-3))
    lam<-lambda^sigma
    muu<-mu^sigma
...
doCMPrediction<-function(df, xi=0, currentDate=Sys.Date()){
    ...
    res <- optim(par=par.inits, fn=CMoptimFn, DCm=dcm, xi=xi, currentDate=currentDate, method='Nelder-Mead')
