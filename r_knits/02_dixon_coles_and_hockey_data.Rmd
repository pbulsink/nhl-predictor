...
t<-proc.time()
nhl.all.res<-doDCPrediction(nhl_all, xi=0.005)
t_all_calc<-proc.time()-t
```

```{r xi_one_res, warning=FALSE, cache=TRUE, echo=FALSE}
t<-proc.time()
nhl.one.res<-doDCPrediction(nhl2015, xi=0.005)
t_one_calc<-proc.time()-t
```
...
team_params<-data.frame("Team"=nhl_teams, "Attack"=0, "Defence"=0)
for (t in 1:length(as.character(unique(nhl2015$AwayTeam)))){
     team<-as.character(unique(nhl2015$AwayTeam))[t]
     team_params[team_params$Team == team,]$Attack=res_2015$par[paste("Attack", team, sep=".")]
     team_params[team_params$Team == team,]$Defend=res_2015$par[paste("Defence", team, sep=".")]
}
