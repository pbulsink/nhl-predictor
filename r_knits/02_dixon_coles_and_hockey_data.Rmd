...
Use of this is to feed in the data into the `doDCPrediction` function, returning the `res` parameter object. This is a long calculation. With that fed into the predictResult function, with two teams and an optional modification of the `maxgoal` parameter, one can get the chances of a home win, draw (which would lead to overtime) or away win.
...
```{r warning=FALSE, cache=TRUE, echo=FALSE}
t<-proc.time()
nhl.all.res<-doDCPrediction(nhl2015)
