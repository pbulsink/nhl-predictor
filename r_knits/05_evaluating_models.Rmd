...
Now, lets look at the rps results to see how they compare. For 2015 only data in the model, our RPS value is `r rps_2015$rps`, and for all the available data, it's `r rps_all$rps`. Not a huge difference. But, lets' start adding in some $\xi$ values! Again, this is a long calculation

```{r optimizer_xi, cache=TRUE}
xi_choice=c(0.001, 0.015, 0.020, 0.025)

get_rps_with_xi<-function(xi, train, schedule, result, currentDate=Sys.Date(), maxgoal=8){
  r<-doDCPrediction(train, xi=xi, currentDate=currentDate)
  adh<-as.matrix(getADHSeason(schedule, r, maxgoal=8))
  return(rps(unlist(result), adh))
}

rpslist<-vector("list", length(xi_choice))
names(rpslist)<-lapply(xi_choice, function(x){paste("xi", x, sep=".")})

for (i in 1:length(xi_choice)){
    rpslist[paste("xi",xi_choice[i], sep=".")] <- get_rps_with_xi(xi_choice[i], train_2015, test_schedule, test_results, currentDate=as.Date('2014-12-31'))
}
```

Ok, we see that we get RPS values of `r unlist(rpslist)` for xi of `r xi_choice`. But can we optimize that better?
