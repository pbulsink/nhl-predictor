...
```{r poisson_2016, cache=TRUE}
res_2016<-doDCPrediction(nhl2016)
```

If we do that once, we can get a nice stats table, as we'd find on the NHL website:
```{r predict_remainder, cache=TRUE}
...
* Note that there is a tiebreaker not currently built in whereby head to head results matter for two tied teams. This isn't usually required for playoff make or break decisions.