...
doDCPrediction<-function(df, xi=0){
    ...
    res <- optim(par=par.inits, fn=DCoptimFn, DCm=dcm, xi=xi, method='Nelder-Mead')
...
```{r echo=TRUE, cache=TRUE, cache.extra=tools::md5sum('../data/leagues_NHL_2016_games_games.csv')}
new_stdev<-function(stdev, mean, pop, new_val){
    new_mean = (mean*pop + new_val)/(pop+1)
    return(sqrt((((pop-2)*stdev^2) + (new_val - new_mean)*(new_val-mean))/(pop-1)))
}

point_predict<-function(res, schedule, stats, past_results, n=10000, maxgoal=8){
    pp<-matrix(0, nrow=length(unique(stats$Team)), ncol=4)
    rownames(pp)<-sort(unique(stats$Team))
    colnames(pp)<-c('Points', 'StDev', 'Playoffs', 'Presidents')
    sims=0
    scores<-predictRemainderOfSeason(res=res, schedule=schedule, stats=stats, maxgoal=maxgoal)
    stats_table<-makeStatsTable(rbind(past_results, scores))
    standings<-buildStandingsTable(stats_table)
    east_standings<-getConferenceStandings(standings, "East")[1:8,]
    west_standings<-getConferenceStandings(standings, "West")[1:8,]
    sims=1
    pp[,"Points"]<-stats_table[order(stats_table$Team),]$P
    pp[rownames(east_standings),"Playoffs"]<-1
    pp[rownames(west_standings),"Playoffs"]<-1
    #table for playoffs from east_ and west_ standings
...
rownames(results)<-rownames(nhl_2016_predicted_standings)
