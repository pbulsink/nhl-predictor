...
plot_AG<-plot_AG + ggtitle("Away Goals")
plot_AG<-plot_AG + scale_x_continuous(breaks=seq(0,10,2))
plot_AG<-plot_AG + geom_bar(width=0.8)

plot_HG<-ggplot(nhl2015, aes(HG)) 
plot_HG<-plot_HG + ggtitle("Home Goals") 
plot_HG<-plot_HG + scale_x_continuous(breaks=seq(0,10,2))
plot_HG<-plot_HG + geom_bar(width=0.8)

plot_diff<-ggplot(nhl2015, aes(AG-HG)) 
plot_diff<-plot_diff + ggtitle("Goal Difference (+ve is Away Win)")
plot_diff<-plot_diff + scale_x_continuous(breaks=seq(-8,8,2))
plot_diff<-plot_diff + geom_bar(width=0.8)
...
```{r echo=FALSE}
sim_goal_dist<-ggplot(, aes(rpois(nrow(nhl2015)*10, median(nhl2015$AG)))) 
sim_goal_dist<-sim_goal_dist + ggtitle("Simulated Goals")
sim_goal_dist<-sim_goal_dist + geom_bar(width=0.8)
sim_goal_dist
```
...
```{r cache=TRUE}
getAndPrepAllData<-function(yearlist=c(2006, 2007, 2008, 2009,2010,2011,2012,2013,2014,2015,2016)){
    df<-data.frame(Date=NULL, Visitor=NULL, G=NULL, Home=NULL, G.1=NULL, X=NULL)
    for (year in 1:length(yearlist)){
        df<-rbind(df, read.csv(paste('../data/leagues_NHL_',yearlist[year],'_games_games.csv', sep=''))[c("Date", "Visitor", "G", "Home", "G.1", "X")])
    }
...
```{r echo=FALSE}
plot_AG<-ggplot(all_nhl, aes(AG)) 
plot_AG<-plot_AG + ggtitle("Away Goals")
plot_AG<-plot_AG + scale_x_continuous(breaks=seq(0,10,2))
plot_AG<-plot_AG + geom_bar(width=0.8)

plot_HG<-ggplot(all_nhl, aes(HG)) 
plot_HG<-plot_HG + ggtitle("Home Goals") 
plot_HG<-plot_HG + scale_x_continuous(breaks=seq(0,10,2))
plot_HG<-plot_HG + geom_bar(width=0.8)

plot_diff<-ggplot(all_nhl, aes(AG-HG)) 
plot_diff<-plot_diff + ggtitle("Goal Difference (+ve is Away Win)")
plot_diff<-plot_diff + scale_x_continuous(breaks=seq(-10,10,2))
plot_diff<-plot_diff + geom_bar(width=0.8)
